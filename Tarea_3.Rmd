...
ggplot()+
  geom_line(aes(x,y), color="green", linewidth=1.5)+
...
probabilidad_j <- function(Vi, Vj, B = 1) {
  ...
  ggplot() +
    geom_function(fun = fv_den, color = "orange", linewidth = 0.5, xlim = c(-B - 0.1, B + 0.1)) +
    ...
    labs(title = "Probabilidad de elecciónj") +
    theme_bw()
...
ggplot() +
  ...
  geom_function(fun = fv_den, color = "yellow", linewidth = 0.5) +
  xlim(-1.5, 1.5) +
...
decision <- function(Vj, Vi, B = 1) {
  ...
  ggplot() +
    geom_function(fun = fv_den, color = "red", linewidth = 0.5, xlim = c(-B - 0.1, B + 0.1)) +
    stat_function(fun = fv_den, geom = "area", fill = "blue", alpha = 0.5, xlim = c(-B, Vji)) +
